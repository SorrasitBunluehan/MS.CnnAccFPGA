...
architecture behav of ACCU is
    ...
    signal output_size_reg : unsigned(OUTPUT_SIZE_BIT_WIDTH-1 downto 0);
    signal cur_out_inst_count, cur_out_inst_count_delay , nxt_out_inst_count : integer range 0 to 255;
    
    
    --signal input_size_reg : unsigned(INPUT_SIZE_BIT_WIDTH -1 downto 0);
    --signal input_depth_reg : unsigned(INPUT_DEPTH_BIT_WIDTH-1 downto 0);
    --signal kernel_size_reg : unsigned(KERNEL_SIZE_BIT_WIDTH-1 downto 0);
    --signal kernel_depth_reg : unsigned(KERNEL_DEPTH_BIT_WIDTH-1 downto 0);
    --signal stride_reg : unsigned(STRIDE_BIT_WIDTH-1 downto 0);
    --signal hw_acc_en_reg : std_logic;
    --signal af_en_reg : std_logic;

begin

    --------------------------
    -- Network Parameter Reg
    --------------------------
    --process(clk)
    --begin
    ...
    --            input_depth_reg <= (others => '0'); 
    --            kernel_size_reg <= (others => '0'); 
    --            kernel_depth_reg <= (others => '0');
    --            stride_reg <= to_unsigned(1,stride_reg'length); 
    --            hw_acc_en_reg <= '0';
    --            af_en_reg <= '0';
    --        else 
    --            input_size_reg      <= input_size;
    --            input_depth_reg     <= input_depth;
    --            kernel_size_reg     <= kernel_size;
    --            kernel_depth_reg    <= kernel_depth;
    --            stride_reg          <= stride;
    --            hw_acc_en_reg       <= hw_acc_en;
    --            af_en_reg           <= af_en; 
    --            output_size_reg      <= output_size;
    --        end if;
    --    end if;
    --end process;
