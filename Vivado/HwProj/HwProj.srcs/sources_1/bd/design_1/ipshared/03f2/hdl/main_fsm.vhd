...
architecture behav of main_fsm is
	...
	type state_type is (IDLE, W_PREP, X_PREP, L_X, READ_INDEX,  L_W, COMPUTE, WAIT_ALU, CLEAR_REG);
...
begin
    ...
    alu_ready <= alu_valid0 and alu_valid1;
	...
	------------------------------------------------------------------------
	NEXT_STATE_DECODE:
		process (hw_acc_en,output_size, input_size, stride, c_t_f, c_state, w_addr_c, last_input, tvalid, tlast, x_col_cur, x_row_cur, pixel_last, x_prep_done, setzero, kernel_depth, alu_ready)
		begin
			n_state <= c_state;
			if setzero = '1' then
				n_state <= IDLE;
